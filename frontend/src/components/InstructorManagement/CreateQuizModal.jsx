...
import { Steps, Button, Form, Input, Select, InputNumber, Card, Radio, Space, Checkbox } from 'antd'
...
const CreateQuizModal = ({ onClose, onSubmit }) => {
  ...
  const handleNext = async () => {
    if (currentStep === 0) {
      try {
        const values = await form.validateFields(['title', 'course', 'timeLimit', 'passingScore'])
  ...
  const handleSaveDraft = () => {
    const allData = { ...quizData, questions, status: 'draft' }
    onSubmit(allData)
    handleReset()
  }
  ...
  const handlePublish = async () => {
    try {
      ...
      const allData = { ...quizData, questions, status: 'active' }
      onSubmit(allData)
      handleReset()
    } catch (error) {
      ...
      <Form form={form} layout="vertical">
        ...
        {currentStep === 0 && (
          <Card style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12 }}>
            ...
            >
              <Select placeholder="Choose a course..." size="large">
                ...
                <Option value="nodejs-backend">Node.js Backend Development</Option>
              </Select>
            </Form.Item>
          ...
          <Card style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12 }}>
            ...
            <Card style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, marginBottom: 24 }}>
              ...
              <div style={{ color: '#1e40af', lineHeight: 2 }}>
                ...
                <div><strong>Course:</strong> {quizData.course || 'Not selected'}</div>
