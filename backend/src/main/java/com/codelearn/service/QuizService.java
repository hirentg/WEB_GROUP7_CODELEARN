...
public class QuizService {
     ...
     */
    public List<QuizDetailResponse> getQuizzesByCourseId(String courseId) {
        List<Quiz> quizzes = quizRepository.findByCourseIdAndPublished(courseId);
    ...
    /**
     * Get quiz detail by ID for student (no instructor check)
     */
    public QuizDetailResponse getQuizDetailForStudent(Long quizId) {
        ...
        Video video = videoRepository.findById(quiz.getVideoId()).orElse(null);
        String courseId = video != null ? video.getCourseId() : null;
    ...
    public Quiz updateQuiz(Long id, CreateQuizRequest request, Long instructorId) {
        ...
        quiz.setStatus(
                request.getStatus() != null && request.getStatus().equalsIgnoreCase("active") ? "ACTIVE" : "DRAFT");
                    ...
                    if (questionReq.getOptions() != null) {
                        for (int i = 0; i < questionReq.getOptions().size(); i++) {
                            ...
                            QuizOption option = new QuizOption(
                                    ...
                                    questionReq.getCorrectAnswerIndex() != null
                                            && questionReq.getCorrectAnswerIndex() == i,
                                    i);
